...
# for i in range(3):
#     for j in range(len(df1['x'])//100):
#         axes[i].axvline(j*100)
        # axes[0][0].axvline(n*100)
        
    # axes[i].set_xlim(2000, 2500)



# axes[0].plot(df1['TS'], df1['x'], label="X1")
# axes[0].set_ylabel("X")
# axes[0].legend(loc="upper right")

# axes[1].plot(df1['TS'], df1['y'], label="Y1", color="tab:orange")
# axes[1].set_ylabel("Y")
# axes[1].legend(loc="upper right")

# axes[2].plot(df1['TS'], df1['z'], label="Z1", color="tab:green")
# axes[2].set_ylabel("Z")
...
# cont = (df1['TS'][99] - df1['TS'][0])/100
# for n in range(len(df1['TS'])):
#     discont = df1['TS'][n+1] - df1['TS'][n]
#     if discont > cont + .005:
#         print(f'Discontinuity at {n} of {discont-cont}')
...
print(f'mean: {conts.mean()}')
print(f'median: {conts.median()}')
print(f'mode: {conts.mode()}')
